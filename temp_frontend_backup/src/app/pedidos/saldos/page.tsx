...
export default function SaldosCreditosPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Armazena histórico de logs de ajustes
  ...
  const getProductionTimeDisplay = (startStr: string, endStr: string) => {
  ...
  const filteredAdjustments = adjustments.filter(a => {
    const matchCustomer = filterCustomer ? a.customer_id === filterCustomer : true;
    const matchProduct = filterProduct ? a.product_id === filterProduct : true;
    const matchSearch = searchQuery ? (
      a.customer?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      a.product?.name?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      a.order?.pv_number?.toLowerCase().includes(searchQuery.toLowerCase())
    ) : true;
    return matchCustomer && matchProduct && matchSearch;
  });
