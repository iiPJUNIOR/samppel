  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const result = await service.importOrders(startDate, endDate, sendProgress);
        ...
        controller.enqueue(encoder.encode(JSON.stringify({ success: false, error: err.message || 'Erro ao importar pedidos.' }) + '\n'));
      } finally {
        controller.close();
      }
    }
