...
export async function POST(request: NextRequest) {
  try {
    ...
    // Disparamos a importação do pedido em background / assíncrona para responder rápido à Conta Azul
    // (A Conta Azul exige resposta HTTP 200/204 rápida sob pena de desativar o webhook)
    (async () => {
      try {
    ...
    // Retorna imediatamente com sucesso para a Conta Azul
    return NextResponse.json({ success: true, message: 'Notification received successfully' });
  } catch (err: any) {
